...
namespace BlogApp.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        {
            var oldCategory=await _unitOfWork.Categories.GetAsync(c=>c.Id==categoryUpdateDto.Id);// bizim categoryUpdate dto içindeki created by name modified by name verilerimiz bulunyor ama aşşğıda category nesnesinde bu veriler yok biz burda o verileri aldık aşşağıda ekledik.
            var category = _mapper.Map<CategoryUpdateDto,Category>(categoryUpdateDto,oldCategory);//category nin içine Category içinde olmayan verileri yukarıdaki satırda categoryUpdateDto dan gelen verileride alarak oldCategory içien atıp bunuda category değişkenine attık.
        ...
        {
            var result=await _unitOfWork.Categories.AnyAsync(_ => _.Id == categoryId);             
            if (result==true)
            {

                var category = await _unitOfWork.Categories.GetAsync(c => c.Id == categoryId);//veritabanında veri yi çektik ama category türünde çektik 
                var categoryUpdateDto=_mapper.Map<CategoryUpdateDto>(category);// sonra category türünde çektiğimiz veriyi categoryUpdateDto türüne Automapper ile çevirdik ve geri döndürdük
