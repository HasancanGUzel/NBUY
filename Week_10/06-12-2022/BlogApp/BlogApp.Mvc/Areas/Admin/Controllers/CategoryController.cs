...
namespace BlogApp.Mvc.Areas.Admin.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        {
            var result = await _categoryService.GetCategoryUpdateDto(categoryId);  //burada kategori sayfamızade düzelt butonuna tıklandığı zaman tıklanılan satırdaki (tr) verileri form ekranına getirmek için yazdık
            if (result.ResultStatus==ResultStatus.Success)
            {
        ...
        public async Task<IActionResult> Update(CategoryUpdateDto categoryUpdateDto)
        {
            if (ModelState.IsValid)
            {
                var result = await _categoryService.Update(categoryUpdateDto, "Hasancan Güzel");
                if (result.ResultStatus==ResultStatus.Success)
                {
