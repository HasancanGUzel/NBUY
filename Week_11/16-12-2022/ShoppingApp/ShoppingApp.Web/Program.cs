...
builder.Services.AddDbContext<IdentityContext>(options=>options.UseSqlite(builder.Configuration.GetConnectionString("SqliteConnection")));// burada appsettings.json dosyasýnda tanýmladýðýmýz veritabaný baðlantýsýný kullnadýk kendimiz içine data soruce falan yazmadýk

builder.Services.AddDbContext<ShopAppContext>(options => options.UseSqlite(builder.Configuration.GetConnectionString("SqliteConnection")));// ShopAppContext deki  onconfiguration metodunu kaldýrdýk ve yukarýdaki gibi baðlantýyý  appsettings dan aldýk
// bunu ShopAppContext de karþýlýcak ctro tanýmladýk base olarak aldýk
...
builder.Services.AddScoped<IEmailSender, SmtpEmailSender>(x => new SmtpEmailSender(
    ...
    builder.Configuration.GetValue<int>("EmailSender:Port"),//sayýsal deðer olduðu için GetValue<int> yaptýk
    builder.Configuration.GetValue<bool>("EmailSender:EnableSSl"),
    builder.Configuration["EmailSender:UserName"],
    builder.Configuration["EmailSender:Password"]

    )); // bunuda burada yazmak yerine appsettings.json da tanýmladýk
