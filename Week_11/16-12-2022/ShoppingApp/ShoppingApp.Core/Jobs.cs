...
namespace ShoppingApp.Core
{
    public static class Jobs
    {
        ...
        //bu metodu kullanarak mesaj verdirdiğimiz yerden bunu kullnıcaz
        public static string CreateMessage(string title,string message,string alertType)
        {
            var msg = new AlertMessage // AlertMessage nesne türettik ve içine dışarıdan gelen verileri aktardık 
            {
                Title = title,
                Message = message,
                AlertType = alertType
            };
            return JsonConvert.SerializeObject(msg); // JsonConvert burada çıkmıyordu bizde üstüne tıklayarak package indirdik
        }
    }
}
