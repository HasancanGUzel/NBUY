    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterDto registerDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //yeni kayıt olan kullancııya varsayılan olarak user rolünüü veriyoruz
                    await _userManager.AddToRoleAsync(user, "User");

                    //yeni kayıt olan kullancııya bir alışveriş sepeti oluşturuyoruz.
                    await _cardManager.InitializeCard(user.Id);


                    TempData["Message"] = Jobs.CreateMessage("Bilgi", "Hesabınız başarı ile oluşturulmuştur", "success");
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string token)
        {
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("Login","Account");
                    // normalde mail onayı olsaydı burada card oluşturabilrdik ama biz mail oanyını kldırıcaz ve cardı kayıt ol register da oluşturucaz
                }
